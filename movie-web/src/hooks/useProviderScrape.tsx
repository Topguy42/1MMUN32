...
function useBaseScrape() {
  ...
  const noVerifiedProvidersEvent = useCallback(() => {
    const id = "verified-providers";
    setSources({
      [id]: {
        id,
        name: "Verified playback providers",
        status: "failure",
        reason:
          "No verified playback providers are configured. The default archive sources are disabled because they are blocked, dead, or unverified from this network.",
        percentage: 100,
      },
    });
    setSourceOrder([{ id, children: [] }]);
    setCurrentSource(id);
  }, []);
...
export function useScrape() {
  ...
  const startScraping = useCallback(
    async (media: ScrapeMedia) => {
      ...
      startScrape();
      if (!hasEnabledPlaybackProviders()) {
        noVerifiedProvidersEvent();
