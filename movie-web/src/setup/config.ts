...
const env: Record<keyof Config, undefined | string> = {
  ...
  ENABLED_PROVIDER_IDS: import.meta.env.VITE_ENABLED_PROVIDER_IDS,
  ENABLED_EMBED_IDS: import.meta.env.VITE_ENABLED_EMBED_IDS,
...
function getCsvKey(key: keyof Config): string[] {
  return getKey(key, "")
    .split(",")
    .map((v) => v.trim())
    .filter((v) => v.length > 0);
...
export function conf(): RuntimeConfig {
  ...
  return {
    ...
    ENABLED_PROVIDER_IDS: getCsvKey("ENABLED_PROVIDER_IDS"),
    ENABLED_EMBED_IDS: getCsvKey("ENABLED_EMBED_IDS"),
