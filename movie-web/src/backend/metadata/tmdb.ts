...
const tmdbCredential = conf().TMDB_READ_API_KEY;
const isV3ApiKey = /^[a-f0-9]{32}$/i.test(tmdbCredential);

const headers = {
  accept: "application/json",
  ...(isV3ApiKey ? {} : { Authorization: `Bearer ${tmdbCredential}` }),
};
...
async function get<T>(url: string, params?: object): Promise<T> {
  const res = await mwFetch<any>(encodeURI(url), {
    ...
    params: {
      ...params,
      ...(isV3ApiKey ? { api_key: tmdbCredential } : {}),
    },
  });
  return res;
}
